...
It is not recommended to set your `ownerField` to be the same as your `@primaryKey` field, or `id` if no primary key is specified. If you wish to be able to query by the `ownerField` specified, using an `@index` on that `ownerField` is recommended instead.
